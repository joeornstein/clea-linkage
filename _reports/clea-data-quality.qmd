...
The issues are grouped into three categories. An additional systematic comparison of all CLEA `sub` values against ISO 3166-2 subdivision names identified five further sub-field typos (Austria, Liberia, Thailand, Uganda, Zimbabwe); these are documented in @sec-other-sub and corrected upstream in the cleaning script rather than in the validated dataset.
...
### Other countries: Sub field typos {#sec-other-sub}

A systematic comparison of all CLEA `sub` values against ISO 3166-2 subdivision names identified five additional countries where the `sub` field contains an invalid or garbled string. Unlike the Germany and Canada errors above — where `sub` names a *valid* subdivision but the *wrong* one for that constituency — these cases involve values that do not correspond to any ISO subdivision at all.

All five are corrected in `001_clean-data.R` before linking.

```{r}
#| label: tbl-sub-typos
#| tbl-cap: "Sub field string errors identified by systematic scan"

tribble(
  ~Country,  ~`Erroneous sub`,                          ~`Correct value`,   ~Notes,
  "Austria",  "Wine",                                    "Wien",             "Typo; affects all Wien constituencies across all election years",
  "Liberia",  "Rivercress",                              "River Cess",       "Misspelling; ISO LR-RI is 'River Cess'",
  "Thailand", "Bankgok",                                 "Bangkok",          "Typo; 'Bangkok' also appears correctly in other years",
  "Uganda",   "LYAlliance for National Transformation",  "LYANTONDE",        "Party-name text concatenated into sub field; affects the KABULA constituency in 2021 only",
